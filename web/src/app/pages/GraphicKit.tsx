...
function HeroCard() {
  return (
    <div
      className="relative overflow-hidden rounded-3xl shadow-2xl w-full max-w-[540px] h-[460px] md:h-[540px]"
      style={{ background: "linear-gradient(135deg, #a855f7 0%, #ec4899 40%, #f97316 80%, #fbbf24 100%)" }}
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full px-8 text-center">
        ...
        <h1 className="text-white leading-tight mb-2" style={{ fontFamily: "'Jua', sans-serif", fontSize: "clamp(2rem, 8vw, 2.8rem)" }}>마음의 편지함</h1>
...
function StoryCard({ nickname }: { nickname: string }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl shadow-2xl flex flex-col w-full max-w-[540px] md:max-w-[280px]" style={{ height: 340, background: "linear-gradient(160deg, #1e1b4b 0%, #312e81 35%, #7c3aed 70%, #c026d3 100%)" }}>
...
function FeatureBannerCard({ dark }: { dark: boolean }) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-3xl shadow-2xl transition-colors duration-500 flex flex-col p-5 justify-between w-full max-w-[540px] min-h-[430px] md:h-[510px]"
      style={{
        background: dark
...
function WritingCard({ dark }: { dark: boolean }) {
  return (
    <div
      className="relative overflow-hidden rounded-3xl shadow-2xl transition-colors duration-500 w-full max-w-[540px] md:max-w-[280px]"
      style={{
        height: 176,
        background: dark
...
function AdminCard() {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-3xl shadow-2xl cursor-pointer w-full max-w-[540px] md:max-w-[280px]"
...
function ActivityCard({ dark }: { dark: boolean }) {
  const [data, setData] = useState<{
    totalUsers: number;
    totalPosts: number;
    weeklyStats: { week: number; visitorCount: number; postCount: number }[];
  }>({
    totalUsers: 0,
    totalPosts: 0,
    weeklyStats: []
  ...
  useEffect(() => {
    fetchApi("/stats")
      .then((res) => {
        setData(res || { totalUsers: 0, totalPosts: 0, weeklyStats: [] });
        if (res?.weeklyStats && res.weeklyStats.length > 0) {
          setSelectedWeek(res.weeklyStats[res.weeklyStats.length - 1].week);
        }
      })
      .catch(console.error);
  }, []);

  const selectedStat = data.weeklyStats.find((w) => w.week === selectedWeek) || {
    week: selectedWeek || 1,
    visitorCount: 0,
    postCount: 0
  };
  ...
  const statItems = [
    {
      label: `${selectedWeek || 1}주차 접속자`,
      val: `${selectedStat.visitorCount}명`,
      icon: Users,
      textColor: "text-blue-400",
      iconColor: "rgba(59,130,246,0.15)"
    },
    {
      label: `${selectedWeek || 1}주차 편지수`,
      val: `${selectedStat.postCount}개`,
      icon: Mail,
  ...
  const maxVal = Math.max(...data.weeklyStats.map((w) => Math.max(w.visitorCount, w.postCount)), 1);
  const maxBarHeight = 36;
  const baselineY = 50;

  const visitorColor = dark ? "url(#visitorGradDark)" : "url(#visitorGradLight)";
  const postColor = dark ? "url(#postGradDark)" : "url(#postGradLight)";

  const colCount = Math.max(data.weeklyStats.length, 1);
  const colWidth = 240 / colCount;

  const renderBars = () => {
    if (data.weeklyStats.length === 0) {
      return (
        <text x="120" y="35" textAnchor="middle" fill={dark ? "#a78bfa" : "#7c3aed"} className="text-[10px] font-bold opacity-60">
          데이터가 없습니다.
        </text>
      );
    }

    return data.weeklyStats.map((w, i) => {
      const xStart = i * colWidth;
      const xCenter = xStart + colWidth / 2;

      const barWidth = Math.max(4, Math.min(10, colWidth / 3.5));
      const barSpacing = Math.max(2, Math.min(4, colWidth / 8));

      const vHeight = w.visitorCount > 0 ? (w.visitorCount / maxVal) * maxBarHeight : 2;
      const pHeight = w.postCount > 0 ? (w.postCount / maxVal) * maxBarHeight : 2;

      const vY = baselineY - vHeight;
      const pY = baselineY - pHeight;

      const isSelected = selectedWeek === w.week;
      const itemOpacity = isSelected ? 1 : 0.45;

      return (
        <g key={w.week} style={{ transition: "all 0.3s ease" }}>
          {/* Highlight background for selected week */}
          {isSelected && (
            <rect
              x={xStart + 2}
              y={2}
              ...
              height={66}
              rx="8"
              fill={dark ? "rgba(167, 139, 250, 0.08)" : "rgba(168, 85, 247, 0.05)"}
              stroke={dark ? "rgba(167, 139, 250, 0.2)" : "rgba(168, 85, 247, 0.15)"}
              strokeWidth="1"
            />
          )}
          ...
          <text
            ...
            y={baselineY + 12}
            textAnchor="middle"
            fill={dark ? "#e9d5ff" : "#4c1d95"}
            className="text-[8px] font-black"
            opacity={isSelected ? 0.9 : 0.5}
            style={{ fontFamily: "'Jua', sans-serif" }}
          >
            {colCount <= 3 ? `${w.week}주차` : `${w.week}주`}
          </text>
  ...
  return (
    <div
      className="relative overflow-hidden rounded-3xl shadow-2xl transition-colors duration-500 p-4 flex flex-col justify-between w-full max-w-[540px] md:max-w-[280px]"
      style={{
        height: 320,
        background: dark
    ...
    >
      ...
      <div className="relative z-10 flex flex-col gap-2 h-full justify-between">
        ...
        {/* SVG Graph */}
        <div className="w-full flex items-center justify-center my-0.5" style={{ minHeight: 70 }}>
          <svg width="100%" height="70" viewBox="0 0 240 70">
            <defs>
              <linearGradient id="visitorGradDark" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#60a5fa" />
                <stop offset="100%" stopColor="#3b82f6" />
              </linearGradient>
              <linearGradient id="visitorGradLight" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#3b82f6" />
                <stop offset="100%" stopColor="#1d4ed8" />
              </linearGradient>

              <linearGradient id="postGradDark" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#c084fc" />
                <stop offset="100%" stopColor="#a855f7" />
              </linearGradient>
              <linearGradient id="postGradLight" x1="0" y1="0" x2="0" y2="1">
          ...
          {statItems.map((item, index) => {
            ...
            return (
              <motion.div
                key={index}
                whileHover={{ scale: 1.01, x: 2 }}
                className="px-3 py-1.5 rounded-xl border transition-all flex items-center justify-between shadow-sm"
                style={{
                  background: dark ? "rgba(255,255,255,0.02)" : "rgba(255,255,255,0.65)",
                  borderColor: dark ? "rgba(167,139,250,0.06)" : "rgba(168,85,247,0.06)",
                }}
              >
                <span className="text-[9px] opacity-75 font-bold" style={{ fontFamily: "'Noto Sans KR', sans-serif", color: dark ? '#c4b5fd' : '#4c1d95' }}>
                  {item.label}
                </span>
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-black" style={{ fontFamily: "'Jua', sans-serif", color: dark ? '#e9d5ff' : '#2d1b4e' }}>
                    {item.val}
                  </span>
                  <div
                  ...
                  >
                    <Icon size={11} className={item.textColor} strokeWidth={2.5} />
                  </div>
                </div>
              </motion.div>
            );
    ...
    <div className="w-full flex flex-col items-center mt-20 max-w-4xl">
      <div className="text-center mb-8">
        ...
        <p className="text-sm font-medium opacity-75" style={{ fontFamily: "'Noto Sans KR', sans-serif", color: dark ? "#a78bfa" : "#a855f7" }}>
          항상 이야기방을 빛내주시고 이끌어주시는 소중한 분들입니다.
        </p>
      </div>
      ...
      >
        ...
        {/* Sliders */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full text-xs font-semibold mt-2" style={{ color: dark ? "#c4b5fd" : "#4c1d95" }}>
          <div className="flex flex-col gap-1">
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 px-4">
      ...
      <div className="flex flex-col items-center gap-6 w-full max-w-4xl">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="flex flex-wrap justify-center gap-6 w-full">
          <HeroCard />
          <div className="flex flex-col gap-6 w-full max-w-[540px] md:max-w-[280px]">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.25 }} className="flex flex-wrap justify-center gap-6 w-full">
